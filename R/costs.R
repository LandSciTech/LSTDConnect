...
#' @export
setCosts <-function(composites,landclasses,outDir,outName,costOptions,costScns=NULL,extraProtectionValue=c(),exemptionClasses=c("urban","cropland","cutblock")){
  ...
  for(cc in costScns){
    ...
    if(!is.element(cc,names(costOptions))){
      stop("Cost scenario ",cc," not recognized. Options are: ",paste(setdiff(names(costList$costOptions),c("Landclass","speciesType")),collapse=","))
    }

    if(is.element(cc,names(extraProtectionValue))){
      protectionLevels = extraProtectionValue[[cc]]
    }else{protectionLevels=c(0)}

    aCosts = subset(costOptions,select=c("Landclass","speciesType",cc))
    names(aCosts)[names(aCosts)==cc]="cost"
    aCosts = subset(aCosts,!is.na(cost))
    ...
    for(ee in protectionLevels){
      for(sp in spTypes){
        ...
        }else{outNameC =paste0(c(cc),collapse=".")}
        ...
        cCosts = subset(aCosts,speciesType==sp,select=c("Landclass","cost"))
        ...
        if(ee!=0){
          ...
          cCosts$cost[!grepl(" P",cCosts$Landclass,fixed=T)&(!is.element(cCosts$Landclass,exemptionClasses))]=cCosts$NPCost[!grepl(" P",cCosts$Landclass,fixed=T)&(!is.element(cCosts$Landclass,exemptionClasses))]
          cCosts$NPCost=NULL
        }
        #get LCIDs
