...
setMethod(f = 'initialize', signature = "ParcConnectedness", definition = function(.Object, patches,d_ij,H_j,M_i=list(),Mbar_p=data.frame(),Mbar=data.frame(),metric="") {
...
#' @param habitatValue vector. Vector of H_j values. Indices must correspond to rows(?) of d_ij matrix x. Ignored if x is a RasterStack or ParcConnectedness object.
#' @param distanceMethod Character string. Options are leastCostPaths or ... Argument is ignored if x is a matrix or ParcConnectedness object.
#' @param maxDist Number. Maximum distance between i and j in units of RasterStack x. Ignored if x is a matrix or ParcConnectedness object.
#' @param alpha Number or vector of these. 1/alpha is in units of effective distance. See Drielsma et al 2007 for details.
#' @param metric Character string. Options are "connectivity" or "colonization potential" (equations 3 or 4 of Drielsma)
...
setMethod('parcConnectedness', signature(x="ParcConnectedness"), function(x,habitatValue,distanceMethod,maxDist,alpha,metric,region,memoryLimit,stopOnMemoryLimit) {
  ...
  if(class(x@d_ij)=="matrix"){x$d_ij=list(clump1=x@d_ij)}
  if(class(x@H_j)=="matrix"){x@H_j=list(clump1=x@H_j)}
  ...
  blankMap = x@patches;blankMap[]=NA
  R_i = raster::brick(blankMap);names(R_i)=paste0("alpha",alpha[[1]])

  if(length(alpha)>1){
    for (a in 2:length(alpha)){
       R_i = raster::addLayer(R_i,blankMap);names(R_i)[length(names(R_i))]=paste0("alpha",alpha[[a]])
    }
  }
  ...
  for(cl in clumpSet){
    ...
    for (ca in alpha){
      ...
      #scaleKernel = exponentialKernel(2/ca,cellDim=res(x@patches)[1],returnScale=T,negligible=10^-6)#put in the vicinity of 1
      ...
      print(sort( sapply(ls(),function(aa){object.size(get(aa))})))
      w_ij=t(w_ij)
  ...
  Rbar_p = plyr::ddply(subset(R_all,!is.na(alpha)),plyr::.(alpha,patchId),plyr::summarize,Mbar_p=mean(R_i),patchArea=sum(!is.na(R_i)))
  Rbar = plyr::ddply(Rbar_p,plyr::.(alpha),plyr::summarize,Mbar=sum(Mbar_p*patchArea)/sum(patchArea))
...
setMethod('parcConnectedness', signature(x="RasterStack"), function(x,habitatValue,distanceMethod,maxDist,alpha, metric,region,memoryLimit,stopOnMemoryLimit) {
  ...
  patches=x$patches;x=raster::dropLayer(x,"patches")
  patches[!region]=NA

  isPatches=cellStats(patches,"sum")
  if(isPatches==0){stop("There are no patches for parc calculation A.")}
...
#  cellStats(buffClumps,stat="sum")
  ...
  for (ic in clumps){
    ...
    cCost=x$cost; cCost[buffClumps!=ic]=NA;cCost[is.na(buffClumps)]=NA
    ...
    d_ij=NULL;H_j=NULL;cPatches=NULL #clear memory
    if(length(clumps)==1){
      x=NULL;patches=NULL;buffClumps=NULL;region=NULL
    }
    #sort( sapply(ls(),function(x){object.size(get(x))}))
  ...
  outPC@Mbar = plyr::ddply(outPC@Mbar_p,plyr::.(alpha),plyr::summarize,Mbar=sum(Mbar_p*patchArea)/sum(patchArea))
