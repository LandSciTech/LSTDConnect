...
#' @param bufferedPatches RasterLayer. Optional. If provided maxDist will be 
#'   ignored. bufferedPatches defines the calculation region - use to restrict 
#'   calculations to a portion of the map area.
...
#'   
#' @return 
#' List of effective distance d_ij matrices for each subgraph.
#'   
#' @examples
#' # TO DO: examples
#' 
#' @export
setGeneric("leastCostPathDistances", 
           function(patches, cost, maxDist, bufferedPatches, neighbourhood = "octagon") {
             standardGeneric("leastCostPathDistances")}
)

#' @rdname leastCostPathDistances
setMethod("leastCostPathDistances", signature(patches = "RasterLayer"), 
          function(patches, cost, maxDist, bufferedPatches, neighbourhood) {
            # patches=cPatches; cost=cCost;bufferedPatches=!is.na(cPatches);maxDist=maxDist
            checkAllign <- raster::compareRaster(patches, cost)
            if (!checkAllign) {
              stop("All input rasters must have the same same extent, number of rows and 
         columns, projection, resolution, and origin.")
            ...
            if (isFALSE(all.equal(cRes[1], cRes[2]))) {
              stop("Method assumes square pixels. x and y map resolution must be equal.")
            }
            cost <- cost * cRes[1]
            ...
            for (ic in clumps) {
              ...
              checkMemory <- NULL
              gc()
              checkMemory <- try(matrix(0.0000001, nrow = length(fromV), 
                                        ncol = length(toV) * 1.5), silent = T)
              
              if (inherits(checkMemory, "try-error")) {
                stop(paste("Expecting memory problems. Try reducing focal patch 
                 size or the spatial scale parameter.", checkMemory))
              }
              checkMemory <- NULL
              gc()
              
              # Goal here is to fail gracefully if memory requirements are excessive, 
              # before igraph crashes unceremoniously.
              # The problem is I don't know how much memory igraph really needs. Clearly 
              # larger than output matrix size, but how big?
